<main class="container">
  ...
  <?php echo form_open('usuarios/application'); ?>
    ...
    <div class="form-outline mb-4">
      <select name="moradia" id="moradia" class="form-select" placeholder="<?php echo $this->lang->line('Dwelling'); ?>">
      <option value="" selected disabled><?php echo $this->lang->line('Dwelling'); ?></option>
      <?php foreach ($moradias as $moradia): ?>
              <option value="<?php echo $moradia['id_moradia']; ?>"><?php echo $moradia['moradia']; ?></option>
      <?php endforeach; ?>
      </select>
    </div>

    <div class="form-outline mb-4">
      <select name="horas" id="horas" class="form-select" placeholder="<?php echo $this->lang->line('Hours'); ?>">
      <option value="" selected disabled><?php echo $this->lang->line('Hours'); ?></option>
      <?php foreach ($horas as $hora): ?>
              <option value="<?php echo $hora['id_horassozinho']; ?>"><?php echo $hora['horassozinho']; ?></option>
      <?php endforeach; ?>
      </select>
    </div>

    <div class="form-outline mb-4">
      <select name="outros" id="outros" class="form-select" placeholder="<?php echo $this->lang->line('OtherAnimals'); ?>">
      <option value="" selected disabled><?php echo $this->lang->line('OtherAnimals'); ?></option>
      <?php foreach ($outros as $outro): ?>
              <option value="<?php echo $outro['id_outrosanimais']; ?>"><?php echo $outro['outrosanimais']; ?></option>
      <?php endforeach; ?>
      </select>
    </div>

    <div class="form-floating mb-4">
        <input type="number" id="moradores" name="moradores" class="form-control" placeholder="<?php echo $this->lang->line('Amount_people'); ?>" required>
